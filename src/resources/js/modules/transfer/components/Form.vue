...
const onSubmit = async (test) => {
    ...
    if (route.params.id) {
        await store.dispatch("transfer/updateTransfer", formData.value);
        router.push({ name: "transfer.list" });
    }
    if (!route.params.id) {
        await store.dispatch("transfer/createTransfer", formData.value);
        router.push({ name: "transfer.list" });
    }
};
        ...
        <q-form @submit="onSubmit">
            <div class="row q-col-gutter-md q-mt-xs">
                    ...
                    >
                        ...
                        >
                            <SelectInput
                                ...
                                "
                                :error="$v.from_warehouse_id.$error"
                                :errors="$v.from_warehouse_id.$errors"
                                @input="() => $v.from_warehouse_id.$touch()"
                                @blur="() => $v.from_warehouse_id.$touch()"
                        ...
                        >
                            <DetailInput
                                :warehouse="formData.from_warehouse_id"
                ...
                <CardUpload
                    ...
                    :config="{
                        keyOfImages: 'transfer_documents',
                        maxFiles: 5,
                        allowMultiple: true,
                        allowReorder: true,
