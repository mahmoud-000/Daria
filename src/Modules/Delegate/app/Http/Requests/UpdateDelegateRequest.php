...
class UpdateDelegateRequest extends FormRequest
{
    ...
    public function rules()
    {
        return [
            ...
            'locations.*.name' => ['distinct', 'required', 'string', 'min:3', 'max:100', Rule::unique('locations', 'name')->where('locationable_type', 'Delegate')->ignore($this->delegate->id, 'locationable_id')],
            ...
            'contacts.*.name' => ['distinct', 'required', 'string', 'min:3', 'max:100', Rule::unique('contacts', 'name')->where('contactable_type', 'Delegate')->ignore($this->delegate->id, 'contactable_id')],
            'contacts.*.email' => ['distinct', 'sometimes', 'nullable', 'email', Rule::unique('contacts', 'email')->where('contactable_type', 'Delegate')->ignore($this->delegate->id, 'contactable_id')],
            'contacts.*.phone' => ['distinct', 'sometimes', 'nullable', 'string', Rule::unique('contacts', 'phone')->where('contactable_type', 'Delegate')->ignore($this->delegate->id, 'contactable_id')],
            'contacts.*.mobile' => ['distinct', 'sometimes', 'nullable', 'string', Rule::unique('contacts', 'mobile')->where('contactable_type', 'Delegate')->ignore($this->delegate->id, 'contactable_id')],
