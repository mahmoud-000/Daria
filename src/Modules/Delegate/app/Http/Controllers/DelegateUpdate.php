...
class DelegateUpdate extends Controller
{
    public function __invoke(UpdateDelegateRequest $request, Delegate $delegate)
    {
        try {
            $request = $request->validated();

            $delegate = DB::transaction(function () use ($delegate, $request) {
                $delegate->update(Arr::except($request, ['contacts', 'locations', 'avatar']));
                ...
                if (isset($request['contacts'])) {
                    ...
                    $delegate->contacts()->whereNotIn('id', $ids)->delete();

                    if ($create_contacts) {
                        $delegate->contacts()->createMany($create_contacts);
                    }
                ...
                if (isset($request['locations'])) {
                    ...
                    $delegate->locations()->whereNotIn('id', $ids)->delete();

                    if ($create_locations) {
                        $delegate->locations()->createMany($create_locations);
                    }
                    if ($locations) {
            ...
            return $this->success(__('status.updated', ['name' => $delegate['fullname'], 'module' => __('modules.delegate')]));
