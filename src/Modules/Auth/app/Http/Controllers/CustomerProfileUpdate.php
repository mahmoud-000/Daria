...
class CustomerProfileUpdate extends Controller
{
    ...
    {
        try {
            $profile = Customer::where('id', auth()->id())->first();
            ...
            $profile = DB::transaction(function () use ($profile, $request) {
                ...
                if (isset($request['avatar']) && !is_null($request['avatar']) && !array_key_exists('fake', $request['avatar'])) {
                    (new FilesAssign)($request['avatar'], $profile, 'customers', 'avatar');
                }

                return $profile;
            });

            return $this->success(['loginUser' => AuthResource::make($profile->load(['media'])), 'message' => __('status.updated', ['name' => $profile['username'], 'module' => __('modules.customer')])]);
