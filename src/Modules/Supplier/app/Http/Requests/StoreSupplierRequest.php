...
class StoreSupplierRequest extends FormRequest
{
    ...
    public function rules()
    {
        return [
            ...
            'locations.*.name' => ['distinct', 'required', 'string', 'min:3', 'max:100', Rule::unique('locations', 'name')->where('locationable_type', 'Supplier')->whereNull('locationable_id')],
            ...
            'contacts.*.name' => ['distinct', 'required', 'string', 'min:3', 'max:100', Rule::unique('contacts', 'name')->where('contactable_type', 'Supplier')->whereNull('contactable_id')],
            'contacts.*.email' => ['distinct', 'sometimes', 'nullable', 'email', Rule::unique('contacts', 'email')->where('contactable_type', 'Supplier')],
            'contacts.*.phone' => ['distinct', 'sometimes', 'nullable', 'string', Rule::unique('contacts', 'phone')->where('contactable_type', 'Supplier')],
            'contacts.*.mobile' => ['distinct', 'sometimes', 'nullable', 'string', Rule::unique('contacts', 'mobile')->where('contactable_type', 'Supplier')],
    ...
    {
        return auth()->user()->is_owner || Gate::allows('create-supplier');
    }
