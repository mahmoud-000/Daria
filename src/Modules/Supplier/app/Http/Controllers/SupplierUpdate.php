...
class SupplierUpdate extends Controller
{
    public function __invoke(UpdateSupplierRequest $request, Supplier $supplier)
    {
        try {
            ...
            $supplier = DB::transaction(function () use ($supplier, $request) {
                $supplier->update(Arr::except($request, ['contacts', 'locations', 'avatar']));
                ...
                if (isset($request['contacts'])) {
                    ...
                    $supplier->contacts()->whereNotIn('id', $ids)->delete();

                    if ($create_contacts) {
                        $supplier->contacts()->createMany($create_contacts);
                    }
                ...
                if (isset($request['locations'])) {
                    ...
                    $supplier->locations()->whereNotIn('id', $ids)->delete();

                    if ($create_locations) {
                        $supplier->locations()->createMany($create_locations);
                    }
                    if ($locations) {
            ...
            return $this->success(__('status.updated', ['name' => $supplier['fullname'], 'module' => __('modules.supplier')]));
