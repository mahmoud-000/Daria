...
class PurchaseReturn extends Model
{
    ...
    public static  function boot()
    {
        ...
        static::creating(function ($model) {
            ...
            $model->payment_status = $model->paid_amount == $model->grand_total ? self::PAID : self::UNPAID;
        });
        static::created(function ($model) {
            $model->ref = 'PRI_' . $model->id + 1000;
            $model->save();
        });
        static::updating(function ($model) {
            $model->payment_status =
                round($model->paid_amount, 4) === round($model->grand_total, 4) ? self::PAID : self::UNPAID;
        });
    }
