...
class SaleReturnService implements Defination
{
    use InvoiceTrait;
    ...
    public function claculateUpdateStocky($oldSaleReturnEffected, $detail, $oldDetail, $isComplete, $old_isComplete)
    {

        $stocky = $detail['stocky'];
        if ($oldSaleReturnEffected === self::INVOICE_EFFECTED) {
            if ($isComplete) {
