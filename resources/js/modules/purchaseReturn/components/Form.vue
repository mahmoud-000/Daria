...
const onSubmit = async (test) => {
    ...
    if (route.params.id) {
        await store.dispatch("purchaseReturn/updatePurchaseReturn", formData.value);
        router.push({ name: "purchaseReturn.list" });
    }
    if (!route.params.id) {
        await store.dispatch("purchaseReturn/createPurchaseReturn", formData.value);
        router.push({ name: "purchaseReturn.list" });
    }
};
        ...
        <q-form @submit="onSubmit">
            <div class="row q-col-gutter-md q-mt-xs">
                ...
                <CardUpload
                    ...
                    :config="{
                        keyOfImages: 'purchaseReturn_documents',
                        maxFiles: 5,
                        allowMultiple: true,
                        allowReorder: true,
