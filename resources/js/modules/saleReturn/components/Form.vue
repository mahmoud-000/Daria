...
const onSubmit = async (test) => {
    ...
    if (route.params.id) {
        await store.dispatch("saleReturn/updateSaleReturn", formData.value);
        router.push({ name: "saleReturn.list" });
    }
    if (!route.params.id) {
        await store.dispatch("saleReturn/createSaleReturn", formData.value);
        router.push({ name: "saleReturn.list" });
    }
};
        ...
        <q-form @submit="onSubmit">
            <div class="row q-col-gutter-md q-mt-xs">
                ...
                <CardUpload
                    ...
                    :config="{
                        keyOfImages: 'saleReturn_documents',
                        maxFiles: 5,
                        allowMultiple: true,
                        allowReorder: true,
