...
const onSubmit = async (test) => {
    ...
    if (route.params.id) {
        await store.dispatch("quotation/updateQuotation", formData.value);
        router.push({ name: "quotation.list" });
    }
    if (!route.params.id) {
        await store.dispatch("quotation/createQuotation", formData.value);
        router.push({ name: "quotation.list" });
    }
};
        ...
        <q-form @submit="onSubmit">
            <div class="row q-col-gutter-md q-mt-xs">
                ...
                <CardUpload
                    ...
                    :config="{
                        keyOfImages: 'quotation_documents',
                        maxFiles: 5,
                        allowMultiple: true,
                        allowReorder: true,
