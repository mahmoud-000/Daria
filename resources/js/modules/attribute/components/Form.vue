...
const onSubmit = async () => {
    ...
    if (route.params.id) {
        await store.dispatch("attribute/updateAttribute", formData.value);
        router.push({ name: "attribute.list" });
    }
    if (!route.params.id) {
        await store.dispatch("attribute/createAttribute", formData.value);
        router.push({ name: "attribute.list" });
    }
};
</script>
