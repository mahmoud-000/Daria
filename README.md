...
salary (الراتب)
status (حالة الموظف، مثل "نشط" أو "مستقيل")
departments:

department_id (رقم مسلسل فريد)
department_name (اسم القسم)
manager_id (معرف المدير)
positions:

position_id (رقم مسلسل فريد)
...
salary_id (رقم مسلسل فريد)
employee_id (معرف الموظف)
salary_amount (مبلغ الراتب)
start_date (تاريخ بدء الراتب)
end_date (تاريخ انتهاء الراتب)
leaves:

leave_id (رقم مسلسل فريد)
employee_id (معرف الموظف)
leave_type (نوع الإجازة، مثل "إجازة سنوية" أو "إجازة مرضية")
start_date (تاريخ بدء الإجازة)
end_date (تاريخ انتهاء الإجازة)
reason (سبب الإجازة)
status (حالة الإجازة، مثل "معلقة" أو "مُقرة")
attendances:

attendance_id (رقم مسلسل فريد)
employee_id (معرف الموظف)
date (التاريخ)
status (حالة الحضور، مثل "حاضر" أو "غياب")
reason (سبب الغياب)
performances:

performance_id (رقم مسلسل فريد)
employee_id (معرف الموظف)
...
موظف ينتمي إلى قسم واحد.
موظف يشغل منصب واحد.
موظف قد يكون لديه العديد من الرواتب بمرور الوقت.
موظف قد يطلب العديد من الإجازات.
موظف قد يكون لديه العديد من الحضور.
موظف قد يكون لديه العديد من تقييمات الأداء.
...
7. جدول الوظائف الشاغرة:

vacancy_id (رقم مسلسل فريد)
position_id (معرف المنصب)
department_id (معرف القسم)
start_date (تاريخ بدء الوظيفة)
end_date (تاريخ انتهاء الوظيفة)
description (وصف الوظيفة)
8. جدول المتقدمين للوظائف:

application_id (رقم مسلسل فريد)
vacancy_id (معرف الوظيفة الشاغرة)
...
جداول غير مكررة في نظام إدارة الموارد البشرية (HRMS):
في نظام إدارة الموارد البشرية (HRMS)، هناك بعض الجداول التي يُفضل أن تكون غير مكررة لضمان دقة البيانات وكفاءة تخزينها.

إليك بعض الأمثلة:

1. جدول البلدان:

country_id (رقم مسلسل فريد)
country_name (اسم البلد)
country_code (رمز البلد)
2. جدول المدن:

city_id (رقم مسلسل فريد)
city_name (اسم المدينة)
country_id (معرف البلد)
3. جدول الجامعات:

university_id (رقم مسلسل فريد)
university_name (اسم الجامعة)
country_id (معرف البلد)
4. جدول المهارات:

skill_id (رقم مسلسل فريد)
skill_name (اسم المهارة)
5. جدول أنواع الإجازات:

leave_type_id (رقم مسلسل فريد)
leave_type_name (اسم نوع الإجازة)
6. جدول أنواع الرواتب:

salary_type_id (رقم مسلسل فريد)
salary_type_name (اسم نوع الراتب)
7. جدول أنواع العقوبات:

penalty_type_id (رقم مسلسل فريد)
penalty_type_name (اسم نوع العقوبة)
8. جدول أنواع الحوافز:

incentive_type_id (رقم مسلسل فريد)
incentive_type_name (اسم نوع الحافز)
ملاحظات:

يُستخدم جدول البلدان لتخزين معلومات عن البلدان التي يعمل بها الموظفون أو التي تقع فيها فروع الشركة.
يُستخدم جدول المدن لتخزين معلومات عن المدن التي يعيش فيها الموظفون أو التي تقع فيها فروع الشركة.
يُستخدم جدول الجامعات لتخزين معلومات عن الجامعات التي تخرج منها الموظفون.
يُستخدم جدول المهارات لتخزين معلومات عن المهارات التي يمتلكها الموظفون.
يُستخدم جدول أنواع الإجازات لتخزين معلومات عن أنواع الإجازات المختلفة التي يمكن للموظفين الحصول عليها.
يُستخدم جدول أنواع الرواتب لتخزين معلومات عن أنواع الرواتب المختلفة التي يتقاضاها الموظفون.
يُستخدم جدول أنواع العقوبات لتخزين معلومات عن أنواع العقوبات المختلفة التي يمكن تطبيقها على الموظفين.
يُستخدم جدول أنواع الحوافز لتخزين معلومات عن أنواع الحوافز المختلفة التي يمكن منحها للموظفين.
