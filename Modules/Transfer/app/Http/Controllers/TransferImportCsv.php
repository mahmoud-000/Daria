...
class TransferImportCsv extends Controller
{
    public function __invoke(Request $request)
    {
        if (!auth()->user()->is_owner)  abort_if(Gate::denies('import-csv-transfer'), Response::HTTP_FORBIDDEN, __('permission::messages.gate_denies'));
        try {
            $units = Unit::all();
            $category = Category::all();
            $brand = Brand::all();
            $transfers = $request->all();
            $removed = array_shift($transfers);
            $transfersArray = [];
            foreach ($transfers as $key => $value) {
                ...
                $transfersArray[] = [
                    ...
                    'transfer_desc' => isset($value['transfer_desc']) ? trim($value['transfer_desc']) : null,
                    ...
                    'transfer_unit_id' => isset($value['transfer_unit_id']) ? $this->getMetaId($units, $value['transfer_unit_id'], 'short_name') : null,
                    ...
                    'transfer_type' => isset($value['transfer_type']) ? Arr::first(Transfer::PRODUCT_TYPES, function ($valueInArray, $key) use ($value) {
                        return $valueInArray['name'] == trim($value['transfer_type']);
                    })['id'] : 1,
            ...
            foreach ($transfersArray as $transfer) {
                $validator = Validator::make($transfer, [
                    'name'          => ['required', 'min:3', 'max:100', Rule::unique('transfers', 'name')->withoutTrashed()],
                    'label'         => ['required', 'min:3', 'max:100', 'string', Rule::unique('transfers', 'label')->withoutTrashed()],
                    'transfer_desc'  => ['sometimes', 'max:255', 'nullable', 'string'],
                    'category_id'   => ['sometimes', 'integer', 'nullable'],
                    'brand_id'      => ['sometimes', 'integer', 'nullable'],
                    'barcode'       => ['sometimes', 'max:255', 'nullable', 'string', Rule::requiredIf(!!$transfer['barcode_type'])],
                    'barcode_type'  => ['integer', 'sometimes', 'nullable', Rule::requiredIf(!!$transfer['barcode'])],
                    'currency'      => ['sometimes', 'string', 'nullable'],
                    'cost'          => ['sometimes', 'numeric', 'min:0'],
                    'price'         => ['sometimes', 'numeric', 'min:0'],
                    'unit_id'       => ['nullable', 'integer'],
                    'sale_unit_id'  => ['nullable', 'integer'],
                    'transfer_unit_id' => ['nullable', 'integer'],
                    'tax_type'      => ['integer', 'sometimes', 'nullable', Rule::requiredIf(!!$transfer['tax']), 'integer', Rule::in([1, 2])],
                    'tax'           => ['sometimes', Rule::requiredIf(!!$transfer['tax_type']), 'numeric', 'min:0'],
                    'stock_alert'   => ['sometimes', 'nullable', 'integer', 'min:0'],
                    'is_active'     => ['required', 'boolean'],
                    'transfer_type'    => ['nullable', 'integer', Rule::in([1, 2, 3])],
                    'remarks'       => ['string', 'nullable'],
                ]);

                if ($validator->fails()) {
                    $errors[$transfer['name']] = $validator->errors();
                }
            }
            if ($errors) {
                return $this->error(['errors' => $errors], Response::HTTP_UNPROCESSABLE_ENTITY);
            }
            Transfer::upsert($transfersArray, ['name', 'label']);
