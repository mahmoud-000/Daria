...
class PurchaseImportCsv extends Controller
{
    public function __invoke(Request $request)
    {
        if (!auth()->user()->is_owner)  abort_if(Gate::denies('import-csv-purchase'), Response::HTTP_FORBIDDEN, __('permission::messages.gate_denies'));
        try {
            $units = Unit::all();
            $category = Category::all();
            $brand = Brand::all();
            $purchases = $request->all();
            $removed = array_shift($purchases);
            $purchasesArray = [];
            foreach ($purchases as $key => $value) {
                ...
                $purchasesArray[] = [
                    ...
                    'purchase_desc' => isset($value['purchase_desc']) ? trim($value['purchase_desc']) : null,
                    ...
                    'purchase_type' => isset($value['purchase_type']) ? Arr::first(Purchase::PRODUCT_TYPES, function ($valueInArray, $key) use ($value) {
                        return $valueInArray['name'] == trim($value['purchase_type']);
                    })['id'] : 1,
            ...
            foreach ($purchasesArray as $purchase) {
                $validator = Validator::make($purchase, [
                    'name'          => ['required', 'min:3', 'max:100', Rule::unique('purchases', 'name')->whereNull('deleted_at')],
                    'label'         => ['required', 'min:3', 'max:100', 'string', Rule::unique('purchases', 'label')->whereNull('deleted_at')],
                    'purchase_desc'  => ['sometimes', 'max:255', 'nullable', 'string'],
                    'category_id'   => ['sometimes', 'integer', 'nullable'],
                    'brand_id'      => ['sometimes', 'integer', 'nullable'],
                    'barcode'       => ['sometimes', 'max:255', 'nullable', 'string', Rule::requiredIf(!!$purchase['barcode_type'])],
                    'barcode_type'  => ['integer', 'sometimes', 'nullable', Rule::requiredIf(!!$purchase['barcode'])],
                ...
                if ($validator->fails()) {
                    $errors[$purchase['name']] = $validator->errors();
                }
            }
            if ($errors) {
                return $this->error(['errors' => $errors], Response::HTTP_UNPROCESSABLE_ENTITY);
            }
            Purchase::upsert($purchasesArray, ['name', 'label']);
